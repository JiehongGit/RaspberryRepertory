...
def get_access_token(): 
	...
	"client_id" :"MoeHAV5IOa054LgjHulZ6NZG", #此处填写自己的client_id
	"client_secret":"f39c0f9b8fcd085e500bec5a362bc77a", #此处填写自己的client_secret
	}
	r = requests.post(url,data=body,verify=True)
	respond = json.loads(r.text)
	return respond["access_token"]

def YYrecognition_api(audio_data,token): 
...
def asr_main(filename,tok): 
	try: 
		...
		audio_data = f.read() 
		f.close() 
		resp = YYrecognition_api(audio_data,tok) 
		return resp[0]
	except Exception,e:
                print "e:",e
